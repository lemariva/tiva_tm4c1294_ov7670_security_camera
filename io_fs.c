...
//*****************************************************************************
#include "io_fsdata.h"
static char camerabild[CAM_WIDTH];
//*****************************************************************************
//
...
fs_open(const char *pcName)
{
    ...
    //
	// Process request to CAPTURE_IMAGE
	//
	if(ustrncmp(pcName, "/cgi-bin/capture_image", 22) == 0)
	{
		static char pcBuf[4];
		camera_capture_image();

		usnprintf(pcBuf, 4, "Captured");
    ...
    //
	// Process request to SEND_IMAGE
	//
	if(ustrncmp(pcName, "/cgi-bin/request_image_line?line=", 27) == 0)
	{
		unsigned long ulSpeed;

		char* req_line  = (char*)pcName + 33;
		...
		camera_send_image(camerabild);

		psFile->data  = camerabild;
		psFile->len   = CAM_WIDTH;
		psFile->index = psFile->len;
		psFile->pextension = NULL;
    ...
    //
    else if(ustrncmp(pcName, "/cgi-bin/set_speed?percent=?line=", 12) == 0)
    {
        static char pcBuf[6];
